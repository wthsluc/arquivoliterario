...
class Livro {
    ...
    string $titulo, string $descricao, string $situacao, int $nota, string $capa, int $IdAutor, int $IdCategoria): void {

    $titulo = trim($titulo);
    $situacao = trim($situacao);

    if ($titulo === '' || $situacao === '') {
        throw new InvalidArgumentException('Título e situacao são obrigatórios.');
    }
